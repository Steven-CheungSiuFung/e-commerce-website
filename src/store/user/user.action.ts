...
export type EmailSignUpSuccess = ActionWithPayload<
  USER_ACTION_TYPES.EMAIL_SIGN_UP_SUCCESS,
  { user: User; additionalDetails: AdditionalInformation }
>;

export type EmailSignUpFailed = ActionWithPayload<
  USER_ACTION_TYPES.EMAIL_SIGN_UP_FAILED,
  Error
>;
...
export const emailSignUpStart = withMatcher(
  (displayName: string, email: string, password: string): EmailSignUpStart =>
...
export const emailSignUpSuccess = withMatcher(
  (user: User, additionalDetails: AdditionalInformation): EmailSignUpSuccess =>
    createAction(USER_ACTION_TYPES.EMAIL_SIGN_UP_SUCCESS, {
      user,
      additionalDetails,
    })
);
